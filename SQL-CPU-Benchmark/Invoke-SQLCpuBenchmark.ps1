...
function Get-ServerHardwareInfo {
    ...
    try {
        ...
        $osCpuInfo.CpuLogicalProcessors = ($cpus | Measure-Object -Property NumberOfLogicalProcessors -Sum).Sum
        $osCpuInfo.CpuClockSpeedMHz = ($cpus | Select-Object -First 1).MaxClockSpeed
        # SMT/HT is enabled if logical processors > physical cores
        $osCpuInfo.SmtEnabled = $osCpuInfo.CpuLogicalProcessors -gt $osCpuInfo.CpuCores
    }
    catch {
        Write-BenchmarkLog "Could not get CPU info from OS: $_" -Level "WARNING"
    }
    
    # Try to get EC2 instance type from IMDS
    $ec2InstanceType = "N/A"
    try {
        # Try IMDSv2 first (token-based)
        $token = Invoke-RestMethod -Uri "http://169.254.169.254/latest/api/token" -Method PUT -Headers @{"X-aws-ec2-metadata-token-ttl-seconds" = "21600"} -TimeoutSec 2 -ErrorAction Stop
        $ec2InstanceType = Invoke-RestMethod -Uri "http://169.254.169.254/latest/meta-data/instance-type" -Headers @{"X-aws-ec2-metadata-token" = $token} -TimeoutSec 2 -ErrorAction Stop
    }
    catch {
        try {
            # Fallback to IMDSv1
            $ec2InstanceType = Invoke-RestMethod -Uri "http://169.254.169.254/latest/meta-data/instance-type" -TimeoutSec 2 -ErrorAction Stop
        }
        catch {
...
"@
    ...
    return @{
        ...
        SmtEnabled       = $osCpuInfo.SmtEnabled
...
function Save-ResultsToCsv {
    ...
    $results = foreach ($result in $BenchmarkResults) {
        [PSCustomObject]@{
            ...
            EC2InstanceType  = $HardwareInfo.EC2InstanceType
            ...
            SmtEnabled       = $HardwareInfo.SmtEnabled
...
function Start-Benchmark {
    ...
    if ($hardwareInfo.EC2InstanceType -ne "N/A") {
        Write-Host "  EC2 Instance:     $($hardwareInfo.EC2InstanceType)"
    }
    ...
    Write-Host "  SMT (HT):         $(if ($hardwareInfo.SmtEnabled) { 'Enabled' } else { 'Disabled' })"
